...
- 프론트엔드: React 18, React DOM 18, Zustand
- 빌드/개발 서버: Vite 6, `@vitejs/plugin-react`, ESM(`type: module`)
- 스타일: 순수 CSS (`src/styles/main.css`)
- 상태/저장: Zustand 전역 스토어, `localStorage` 메타 진행도 저장
- API/AI 연동:
  - 클라이언트: `src/apiService.js`
  - 서버: `api/deepseek.js` (DeepSeek API, TTL 캐시, 타임아웃, 폴백)
- 실행 스크립트: `npm run dev/build/preview`, `start.bat`, `stop.bat`
...
- 구현 항목:
  ...
  - [x] 턴 종료 시 선택 근거 로그 기록 (편익/비용 요약)
    - OEM·R&D·마케팅 자동 비교 문장 로그 적용
...
### 우선순위 권장
1. C영역 고도화 완료: 뉴스 전용 모달 + 타임라인 뉴스 마커 적용
2. D영역 고도화 완료: 정책 이벤트 이력 필터(전체/규제/보조금) 적용
3. 밸런스 튜닝 단계: 수요 탄력 계수/정책 충격폭 플레이테스트 기반 보정
